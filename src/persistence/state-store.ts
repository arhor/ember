...
export class StateStore {
  ...
  async load(): Promise<EmberState> {
    ...
    try {
      ...
      throw new StoreUnavailable(errorCode(error) === "ENOENT" ? `continuity store is unavailable: ${this.path}` : `cannot read continuity store ${this.path}: ${errorMessage(error)}`, { cause: error });
    }
    let text: string;
    try { text = decoder.decode(bytes); } catch (error) { throw new StoreUnavailable(`continuity store is not valid UTF-8: ${errorMessage(error)}`, { cause: error }); }
    let state: unknown;
    try { state = JSON.parse(text); } catch (error) { throw new StoreUnavailable(`continuity store is not complete valid UTF-8 JSON: ${errorMessage(error)}`, { cause: error }); }
    validateState(state);
    return state;
  }
  ...
  async acquireWriteLease(): Promise<WriteLease> {
    ...
    const metadata: LockMetadata = { lock_version: 1, owner_token: this.uuid(), pid: this.pid, hostname: this.host, acquired_at: this.now() };
    try {
  ...
  async commit(expectedRevision: number, candidate: EmberState, lease: WriteLease | null = this.lease): Promise<EmberState> {
  ...
  async replaceDocument(state: EmberState) {
    ...
    try {
      ...
      try {
        ...
        throw new DurabilityUncertain("canonical replacement may be visible, but directory synchronization failed; reload and validate the path before making a durability claim", { cause: error });
      }
    } finally {
  ...
  async lockStatus(): Promise<LockStatus> {
    ...
    try {
      ...
      if (errorCode(error) === "EPERM") return { ...parsed, status: "permission_denied", liveness: "indeterminate" };
  ...
  async quarantineStaleLock({ ownerToken, confirmQuiescent }: { ownerToken: string; confirmQuiescent: boolean }) {
    ...
    try {
      ...
      if (errorCode(error) !== "ESRCH") throw new ConcurrentWriter("lock liveness is indeterminate; quarantine refused", { cause: error, diagnosis: second });
    }
...
export async function readLock(path: string): Promise<LockStatus> {
  let bytes: Buffer;
  try { bytes = await readFile(path); } catch (error) { if (errorCode(error) === "ENOENT") return { status: "absent" }; return { status: "indeterminate", error: errorMessage(error) }; }
  let text: string;
  try { text = decoder.decode(bytes); } catch { return { status: "malformed", reason: "lock metadata is not valid UTF-8" }; }
  let metadata: unknown;
  try { metadata = JSON.parse(text); } catch { return { status: "malformed", reason: "lock metadata is not valid JSON" }; }
  const exact = metadata !== null && typeof metadata === "object" && !Array.isArray(metadata) && JSON.stringify(Object.keys(metadata).sort()) === JSON.stringify(["acquired_at", "hostname", "lock_version", "owner_token", "pid"].sort());
  const m = metadata as Partial<LockMetadata>;
  if (!exact || m.lock_version !== 1 || !Number.isSafeInteger(m.pid) || (m.pid ?? 0) <= 0 || typeof m.owner_token !== "string" || !m.owner_token || typeof m.hostname !== "string" || !m.hostname || !isRfc3339Utc(m.acquired_at)) return { status: "malformed", reason: "lock metadata fields are invalid" };
  return { status: "valid", metadata: m as LockMetadata };
}

function errorCode(error: unknown) {
  return error !== null && typeof error === "object" && "code" in error && typeof (error as { code?: unknown }).code === "string" ? (error as { code: string }).code : undefined;
}
