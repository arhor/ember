...
export function codexArgumentEvidence(arguments_: string[]): CodexArgumentEvidence {
  ...
  for (let index = 0; index < arguments_.length; index += 1) {
    ...
    const consumesValue = MODEL_OPTION.has(argument) || CONFIG_OPTION.has(argument) || SENSITIVE_NAME.test(argument);
    if (!argument.startsWith("-") || index + 1 >= arguments_.length || (!consumesValue && arguments_[index + 1]!.startsWith("-"))) continue;
    const value = arguments_[index + 1]!;
    const safeValue = MODEL_OPTION.has(argument) ? bounded(value) : CONFIG_OPTION.has(argument) ? sanitizeConfig(value) : "<redacted>";
...
function splitOption(argument: string): [string, string | undefined] {
  ...
  return index > 0 && argument.startsWith("-") ? [argument.slice(0, index), argument.slice(index + 1)] : [argument, undefined];
}
...
function isModelConfiguration(key: string) {
  return /^(?:model|model_provider|model_reasoning_effort|model_reasoning_summary|model_verbosity|service_tier)$/i.test(key);
}
...
function boundedJson(values: string[], maximum: number) {
  ...
  for (const value of values) {
    if (Buffer.byteLength(JSON.stringify([...kept, value]), "utf8") > maximum) {
      while (kept.length && Buffer.byteLength(JSON.stringify([...kept, "<truncated>"]), "utf8") > maximum) kept.pop();
