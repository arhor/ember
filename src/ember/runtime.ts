...
export function startRuntime(state: EmberState, principal: string, scope: string, { timestamp = nowUtc() }: { timestamp?: string } = {}) {
  ...
  if (typeof scope !== "string" || !scope.trim()) throw new ValidationError("active scope must be non-empty");
...
export function stopRuntime(state: EmberState, runtimeId: RuntimeId | string, { reason, timestamp = nowUtc() }: { reason: string; timestamp?: string }): EmberState {
  const candidate = cloneState(state);
...
export async function runCognition(
  ...
  const projection = buildProjection(state, { principal, scope, currentInput: text, currentTime: timestamp, runtimeId, purpose, explainIds });
  ...
  try {
    ...
    if (current.revision !== state.revision) throw new StaleRevision("canonical revision changed during provider failure", { cause: error });
  ...
  const current = await store.load();
  if (current.revision !== state.revision) throw new StaleRevision("canonical revision changed during provider call");
...
export function findCognition(state: EmberState, id: CognitionId | string): CognitionEpisode {
  ...
  if (!value) throw new ValidationError(`cognition does not exist: ${id}`);
  return value;
}
...
function latestRuntime(state: EmberState): RuntimeEpisode | null {
  ...
  if (tails.length !== 1) throw new ValidationError("runtime recovery chain has no unique current tail");
  return tails[0];
}
...
async function writeOutput(output: Writable | ((text: string) => void | Promise<void>), text: string) {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    try { output.write(text, error => finish(error)); } catch (error) { finish(error instanceof Error ? error : new Error(String(error))); }
  });
}
