...
export async function invokeCodexProvider(
  ...
  if (signal?.aborted) throw new ProviderError("Codex cancellation requested before invocation", { outcome: "cancellation_requested", termination: { reason: "explicit_cancellation", directChildExitObserved: false } });
  ...
  try {
    ...
    const inspectLines = (chunk: Buffer) => {
      ...
      for (const line of lines) {
        try {
          ...
          if (isRecord(event) && event.type === "thread.started" && validExternalId(event.thread_id)) observedThreadId = event.thread_id;
        } catch {}
      }
    };
    ...
    const errorOptions = (outcome: ProviderOutcome, terminationConfirmed = true): ProviderErrorOptions => ({ outcome, terminationConfirmed, externalThreadId: observedThreadId, termination });
    ...
    if (unconfirmed) {
      ...
      const event = terminationReason === "explicit_cancellation" ? "Codex cancellation requested" : terminationReason === "timeout" ? "Codex timed out" : terminationReason === "oversized_stdout" ? "Codex output limit exceeded" : "Codex provider I/O failed";
    ...
    if (spawnError) throw new ProviderError(`Codex is unavailable: ${spawnError.message}`, { ...errorOptions("failed"), cause: spawnError });
    if (terminationReason === "explicit_cancellation") throw new ProviderError("Codex cancellation requested; direct child exit observed but remote work or effects remain unconfirmed", errorOptions("cancellation_requested"));
    if (terminationReason === "timeout") throw new ProviderError(`Codex timed out; direct child exit observed but remote work or effects remain unconfirmed${diagnostic ? `: ${diagnostic}` : ""}`, errorOptions("timed_out"));
    if (terminationReason === "oversized_stdout" || oversized || stdoutBytes > MAX_STDOUT_BYTES) throw new ProviderError("Codex JSONL output exceeds 1 MiB", errorOptions("failed"));
    if (exitCode !== 0) {
      const detail = diagnostic || structuredDiagnostic;
      throw new ProviderError(`Codex exited with ${exitSignal ? `signal ${exitSignal}` : `status ${exitCode}`}${detail ? `: ${detail}` : ""}`, errorOptions("failed"));
    }
...
function parseCodexJsonl(output: string): { result: ProviderResult; externalThreadId?: string } {
  ...
  for (const [index, line] of output.split("\n").entries()) {
    ...
    let event: unknown;
    try { event = JSON.parse(line); } catch (error) { throw new ProviderError(`Codex JSONL line ${index + 1} is invalid JSON: ${errorMessage(error)}`, { cause: error }); }
    if (!isRecord(event) || typeof event.type !== "string" || !event.type.trim()) throw new ProviderError(`Codex JSONL line ${index + 1} is not a typed event object`);
    if (event.type === "thread.started") {
      if (!validExternalId(event.thread_id)) throw new ProviderError("Codex thread.started event has an invalid thread identifier");
      if (externalThreadId !== undefined && externalThreadId !== event.thread_id) throw new ProviderError("Codex JSONL contains inconsistent thread identifiers");
    ...
    if (event.type === "item.completed" && isRecord(event.item) && event.item.type === "agent_message") {
      ...
      agentMessages += 1;
      try { candidate = JSON.parse(event.item.text); } catch (error) { throw new ProviderError(`Codex final agent message is not JSON: ${errorMessage(error)}`, { cause: error }); }
    }
  }
...
function codexErrorDiagnostic(bytes: Uint8Array): string {
  ...
  for (const line of text.split("\n")) {
    try {
      ...
      const message = event.type === "error" && typeof event.message === "string"
        ? event.message
        : event.type === "turn.failed" && isRecord(event.error) && typeof event.error.message === "string"
          ? event.error.message
          : null;
      if (message !== null) return message.replace(/[\u0000-\u001f\u007f]+/g, " ").trim().slice(0, 4096);
    } catch {}
  }
  return "";
}
