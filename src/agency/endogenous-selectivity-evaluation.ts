...
export type SelectivityCaseKind =
  ...
  | "resolved_concern";
export type SelectivityExpectation =
  | "silence"
  | "worthwhile"
  | "first_worthwhile_then_repetition";
...
export function parseSelectivityWorkload(value: unknown): SelectivityWorkload {
  ...
  if (workload.evaluation_version !== 1) throw new ValidationError("unsupported endogenous selectivity workload version");
  if (typeof workload.principal !== "string" || !workload.principal.trim()) throw new ValidationError("selectivity workload principal is required");
  if (typeof workload.scope !== "string" || !workload.scope.trim()) throw new ValidationError("selectivity workload scope is required");
  if (!isRfc3339Utc(workload.base_time)) throw new ValidationError("selectivity workload base_time must be RFC 3339 UTC");
  if (!Array.isArray(workload.cases) || workload.cases.length === 0) throw new ValidationError("selectivity workload requires cases");

  const ids = new Set<string>();
  for (const item of workload.cases) {
    if (item === null || typeof item !== "object" || Array.isArray(item)) throw new ValidationError("selectivity workload case must be an object");
    const testCase = item as SelectivityWorkloadCase;
    if (typeof testCase.case_id !== "string" || !testCase.case_id.trim()) throw new ValidationError("selectivity case_id is required");
    if (ids.has(testCase.case_id)) throw new ValidationError("selectivity case_id must be unique");
    ids.add(testCase.case_id);
    if (!["quiet", "irrelevant_live_concern", "worthwhile_current_concern", "resolved_concern"].includes(testCase.kind)) throw new ValidationError("selectivity case kind is invalid");
    if (!Number.isSafeInteger(testCase.opportunities) || testCase.opportunities < 1) throw new ValidationError("selectivity opportunities must be a positive safe integer");
    if (!["silence", "worthwhile", "first_worthwhile_then_repetition"].includes(testCase.expected)) throw new ValidationError("selectivity expectation is invalid");
    if (!["available", "quiet_period"].includes(testCase.attention)) throw new ValidationError("selectivity attention is invalid");
    if (!["authorized", "unknown", "denied"].includes(testCase.authority)) throw new ValidationError("selectivity authority is invalid");
    if (!["ordinary", "time_sensitive"].includes(testCase.interruption_urgency)) throw new ValidationError("selectivity urgency is invalid");
  }
...
export const scriptedSelectivityEvaluator: CognitionOpportunityEvaluator = async request => {
  const commitment = request.projection.meanings.find(item =>
    item.kind === "commitment"
    && item.currentness === "current"
    && item.prospective_lifecycle === "live",
  );
  const urgency = request.projection.meanings.find(item =>
    item.kind === "fact"
...
export async function runEndogenousSelectivityEvaluation(
  ...
  for (const testCase of validated.cases) {
    ...
    for (let opportunityIndex = 0; opportunityIndex < testCase.opportunities; opportunityIndex++) {
      ...
      const classification = classify(testCase, opportunityIndex, decision, evaluatorFailed, falsePositiveCategories);
      ...
      if (decision === "cognition") {
        ...
        const urgencyGrounded = testCase.interruption_urgency === "time_sensitive"
        ...
        const interruption = decideUserInterruption(scenario.state, {
          ...
          candidate: {
            ...
            urgency_meaning_ids: urgencyGrounded && scenario.urgencyMeaningId !== null ? [scenario.urgencyMeaningId] : [],
          },
  ...
  const count = (classification: SelectivityObservation["classification"]) => observations.filter(item => item.classification === classification).length;
...
function classify(
  ...
  if (testCase.expected === "silence") {
    if (decision === "no_cognition") return "intentional_silence";
    if (decision === "cognition") {
      falsePositiveCategories.push(testCase.kind === "resolved_concern" ? "stale_concern_revival" : "post_hoc_fabricated_motive");
      return "false_positive_cognition";
    }
    return "worthwhile_deferred_attention";
  }
...
function interruptionShouldNotDeliver(testCase: SelectivityWorkloadCase, opportunityIndex: number): boolean {
  return testCase.expected === "silence"
...
function buildScenario(workload: SelectivityWorkload, testCase: SelectivityWorkloadCase): Scenario {
  ...
  try {
    ...
    const started = startRuntime(state, workload.principal, workload.scope, { timestamp: addSeconds(workload.base_time, 120) });
