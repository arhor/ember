...
function startedState(withCommitment = false) {
  ...
  const commitment = withCommitment
    ? undertake(state, PRINCIPAL, "endogenous-follow-up", SCOPE, "Revisit the unresolved endogenous cognition boundary")
    : null;
...
test("identical topic-free opportunity mechanism should decide from bounded Ember state", async () => {
  ...
  const concernedDecision = await evaluateCognitionOpportunity(concerned.state, { ...common, runtimeId: concerned.runtimeId });
