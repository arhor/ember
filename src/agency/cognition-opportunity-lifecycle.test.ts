...
async function cleanup(directory: string, store: StateStore, lease: Awaited<ReturnType<StateStore["acquireWriteLease"]>>) {
  try { await store.releaseWriteLease(lease); } finally { await rm(directory, { recursive: true, force: true }); }
}
...
test("repeated topic-free opportunities should persist intentional silence without fabricating memory", async () => {
  ...
  try {
    ...
    assert.ok(opportunities.every(item => item.selected_meaning_ids.length === 0 && item.interruption_status === "not_attempted"));
    assert.deepEqual([state.meanings.length, state.evidence.length, state.operations.cognition_episodes.length], [0, 0, 0]);
    ...
    try {
      ...
      assert.deepEqual([committed.meanings.length, committed.evidence.length, committed.operations.cognition_episodes.length], [0, 0, 0]);
    } finally {
...
test("durable validator should reject forged silence that claims a selected motive", () => {
  ...
  // When / Then
  assert.throws(() => validateState(state), (error: unknown) => error instanceof ValidationError && /no_cognition must not select/.test(error.message));
});
