...
import { initialState, type MeaningId } from "../core/model.ts";
...
import {
  ...
  type InterruptionDecisionRequest,
...
function fixture() {
  ...
  try {
    commitmentId = undertake(state, PRINCIPAL, "release-preparation", SCOPE, "Prepare the release notes before release");
...
test("internal cognition must not manufacture interruption authority", () => {
  const f = fixture();
  const unknown = decideUserInterruption(f.state, request(f.source, f.commitmentId, f.urgencyId, { authority: "unknown" }));
  const denied = decideUserInterruption(f.state, request(f.source, f.commitmentId, f.urgencyId, { authority: "denied" }));
...
test("only an actually repeated delivered grounding set should be suppressed", () => {
  const f = fixture();
  const repeated = decideUserInterruption(f.state, request(f.source, f.commitmentId, f.urgencyId, {
    previously_delivered_grounding_sets: [[f.urgencyId, f.commitmentId]],
  }));
  const combinedForFirstTime = decideUserInterruption(f.state, request(f.source, f.commitmentId, f.urgencyId, {
  ...
  assert.deepEqual([combinedForFirstTime.outcome, combinedForFirstTime.basis], ["deliver", "current_authorized_candidate"]);
});
...
test("completed cognition may end without a delivery candidate, and absent cognition stays no-delivery", () => {
  const f = fixture();
  const withoutCandidate = decideUserInterruption(f.state, request(f.source, f.commitmentId, f.urgencyId, { candidate: null }));
...
test("candidate and urgency claims must stay inside completed cognition grounding", () => {
  ...
  assert.throws(
    () => decideUserInterruption(f.state, request(f.source, f.commitmentId, f.urgencyId, {
      candidate: {
...
test("interruption requires a completed internal cognition source", () => {
  ...
  assert.throws(
    () => decideUserInterruption(f.state, request(null, f.commitmentId, f.urgencyId, {
      candidate: {
        grounding_meaning_ids: [f.commitmentId],
        urgency: "ordinary",
        urgency_meaning_ids: [],
