import type { CognitionId, EmberState, MeaningId, OpportunityId } from "../model.ts";
...
export function decideUserInterruption(
    ...
    if (
        hasPreviouslyDeliveredGrounding(
...
function hasPreviouslyDeliveredGrounding(grounding: MeaningId[], deliveredSets: MeaningId[][]): boolean {
    const current = new Set(grounding);
    return deliveredSets.some(
        (previous) =>
