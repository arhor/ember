...
export type SetupProvider =
    ...
    | { kind: "deepseek"; model: string; timeoutSeconds: number };
...
export type SetupProbeDiagnostic =
    | {
          event: "probe_started";
          at: string;
          provider: "codex" | "cursor" | "claude-code" | "ollama" | "deepseek";
...
export async function bootstrapContinuity(args: SetupRequest, dependencies: BootstrapDependencies): Promise<number> {
    ...
    if ((kind === "ollama" || kind === "deepseek") && args.providerCommand !== undefined)
        throw new ValidationError("Ollama and DeepSeek do not accept --provider-command");
            ...
            : kind === "deepseek"
              ...
              : {
                    ...
                    command:
                        ...
                        (!changedKind && existing?.provider.kind !== "ollama" && existing?.provider.kind !== "deepseek"
