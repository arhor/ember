...
if (phase === "prepare") {
  ...
  try {
    ...
    const oldConsequence = state.meanings.find(item => item.slot === "release-window" && item.content === "Release is imminent");
    const currentConsequence = state.meanings.find(item => item.slot === "release-window" && item.content === "Release is postponed");
    process.stdout.write(JSON.stringify({
      lineage_id: state.lineage.lineage_id,
      runtime_id: started.runtimeId,
      ...
      supersession: kind === "superseded" && oldConsequence && currentConsequence ? {
