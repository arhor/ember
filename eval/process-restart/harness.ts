...
export async function runProcessRestartScenario(
  ...
  const runArguments = (episode: ScenarioEpisode) => [
    ...
    "--scope", episode.scope,
    "--provider", "codex",
    "--codex-command", options.codexCommand,
    ...((options.codexArguments ?? []).flatMap(value => ["--codex-arg", value])),
    "--provider-timeout-seconds", String(options.timeoutSeconds),
  ];
  ...
  const restartedCurrentAliases = restartedView.current_meanings.map(meaning => reverseAliases.get(meaning.meaning_id) ?? "<unaliased>").sort();
...
function validateProcessRestartScenario(scenario: LongitudinalScenario) {
  ...
  if (baseline.restart_ember || !restarted.restart_ember) throw new Error("process-restart scenario must mark only the second episode as restart_ember");
  if (baseline.external_thread.mode !== "fresh" || restarted.external_thread.mode !== "fresh") throw new Error("process-restart scenario requires fresh provider threads in both episodes");
  if (baseline.cognition_backend !== "codex" || restarted.cognition_backend !== "codex") throw new Error("process-restart scenario currently requires the Codex cognition backend");
  if (baseline.purpose !== undefined || restarted.purpose !== undefined || baseline.explain?.length || restarted.explain?.length) {
    throw new Error("process-restart scenario currently supports ordinary cognition only");
  }
  if (baseline.changes?.length || restarted.changes?.length) throw new Error("process-restart scenario currently supports setup-only durable state");
  for (const action of scenario.setup) {
    if (action.at !== baseline.at) throw new Error("process-restart scenario setup actions must use the baseline episode timestamp");
...
function actionToCli(action: ScenarioAction, principal: string): string {
  switch (action.action) {
    case "remember_relationship":
      return [":remember", "relationship", quoteToken(`relationship:${principal}`), quoteToken(action.scope), quoteToken(action.text)].join(" ");
    case "remember_fact":
      return [":remember", "fact", quoteToken(`user:${principal}`), quoteToken(action.slot), quoteToken(action.scope), quoteToken(action.text)].join(" ");
    case "remember_preference":
      return [":prefer", quoteToken(`user:${principal}`), quoteToken(action.slot), quoteToken(action.scope), quoteToken(action.text)].join(" ");
    case "undertake":
...
function parseSetupAliases(stdout: string, setup: ScenarioAction[]) {
  ...
  for (let index = 0; index < setup.length; index += 1) {
    ...
    if (typeof id !== "string" || !id.startsWith("meaning-")) throw new Error(`setup action ${setup[index].as} did not return a meaning id`);
...
function extractRunReply(stdout: string, setupCount: number) {
  const lines = stdout.split(/\r?\n/);
  return lines.slice(1 + setupCount).join("\n").trim();
}
...
function isInspectionSnapshot(value: unknown): value is InspectionSnapshot {
  ...
  return candidate.lineage !== undefined
...
function requireSuccessfulProcess(label: string, result: ChildResult) {
  ...
  const diagnostic = result.stderr.replace(/[\u0000-\u001f\u007f]+/g, " ").trim().slice(0, 4096);
  throw new Error(`${label} exited with ${result.signal ? `signal ${result.signal}` : `status ${result.code}`}${diagnostic ? `: ${diagnostic}` : ""}`);
}

function replyObservations(episode: ScenarioEpisode, reply: string): ProcessRestartObservation[] {
  return [
    ...(episode.expect.reply_includes ?? []).map(text => observation(
      `${episode.id} reply includes ${JSON.stringify(text)}`,
      true,
      reply.includes(text),
      reply.includes(text),
    )),
    ...(episode.expect.reply_excludes ?? []).map(text => observation(
...
function observation(assertion: string, expected: unknown, observed: unknown, passed: boolean): ProcessRestartObservation {
