...
export async function runEndogenousRestartScenario(
    ...
    const checks = [
        ...
        check(
            "post-restart selected durable reasons",
            scenario.expected_selected_aliases.slice().sort(),
            restarted.selected_aliases?.slice().sort(),
        ),
    ...
    if (scenario.kind === "resolved")
        checks.push(check("resolved concern remains historical", ["concern"], restarted.historical_aliases));
    if (scenario.kind === "superseded") {
        checks.push(
            check(
                "superseded concern-driving reason remains historical",
                ["old-consequence"],
                restarted.historical_aliases,
            ),
            check(
                "replacement reason and still-live concern remain current",
                ["concern", "current-consequence"],
                restarted.current_aliases,
            ),
            check(
                "supersession linkage survives restart",
                {
...
async function runPhase(options: { workerPath: string; cwd: string }, args: string[]): Promise<PhaseResult> {
    return new Promise((resolve, reject) => {
        const child = spawn(process.execPath, [options.workerPath, ...args], {
            ...
            env: {
                ...process.env,
                EMBER_TEST_NOW: args[0] === "prepare" ? "2026-09-03T00:03:00Z" : "2026-09-04T00:00:02Z",
            },
        ...
        child.on("close", (code) => {
            if (code !== 0)
                return reject(
                    new Error(`endogenous restart ${args[0]} phase exited ${code}: ${stderr.trim().slice(0, 4096)}`),
                );
            try {
                resolve({ ...JSON.parse(stdout), pid });
            } catch (error) {
                reject(error);
            }
        });
