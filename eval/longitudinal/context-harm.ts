...
export function evaluateContextHarm(context: ContextEvaluationInput): ContextHarmEvaluation {
  ...
  if (context.omission_candidates.relevant_not_selected.length > 0) {
    omissionFindings.push({
      ...
      rationale: "The scenario declares these meanings relevant, so omitting them creates a material risk of changing justified cognition.",
    });
  }
  ...
  if (forbiddenSelected.size > 0) {
    inclusionFindings.push({
      ...
      rationale: "The projection exposed material the scenario declares forbidden for this recipient or purpose boundary.",
    });
  }

  const purposefulHistorical = [...supersededSelected].filter(alias => relevant.has(alias) && !forbiddenSelected.has(alias));
  if (purposefulHistorical.length > 0) {
    inclusionFindings.push({
      kind: "purposeful_historical_inclusion",
      judgment: "none_observed",
      meanings: purposefulHistorical,
      rationale: "Superseded meaning is relevant to the present historical or explanatory purpose, so its labeled inclusion is not inclusion harm by itself.",
    });
  }

  const staleSelected = [...supersededSelected].filter(alias => !relevant.has(alias) && !forbiddenSelected.has(alias));
  if (staleSelected.length > 0) {
    inclusionFindings.push({
      kind: "stale_inclusion",
      judgment: "material",
      meanings: staleSelected,
      rationale: "Superseded meaning is selected without being relevant to the present purpose, creating material currentness and stale-authority risk.",
    });
  }

  const truthfulDegraded = [...unavailableSelected].filter(alias => unavailableWithGap.has(alias) && !forbiddenSelected.has(alias));
  if (truthfulDegraded.length > 0) {
    inclusionFindings.push({
      kind: "truthful_degraded_inclusion",
      judgment: "none_observed",
      meanings: truthfulDegraded,
      rationale: "The meaning participates together with an explicit unavailable-detail gap, preserving truthful degradation instead of fabricating recall.",
    });
  }

  const unmarkedDegraded = [...unavailableSelected].filter(alias => unavailable.has(alias) && !unavailableWithGap.has(alias) && !forbiddenSelected.has(alias));
  if (unmarkedDegraded.length > 0) {
    inclusionFindings.push({
      kind: "unmarked_degraded_inclusion",
      judgment: "material",
      meanings: unmarkedDegraded,
      rationale: "Unavailable material participates without the projection carrying the corresponding gap, risking false continuity or unsupported certainty.",
    });
  }

  const higherSpecificity = new Set([
    ...forbiddenSelected,
    ...supersededSelected,
    ...unavailableSelected,
  ]);
  const irrelevantSelected = context.inclusion_candidates.irrelevant_selected.filter(alias => !higherSpecificity.has(alias));
  if (irrelevantSelected.length > 0) {
    inclusionFindings.push({
      kind: "irrelevant_inclusion",
      judgment: "potential",
      meanings: irrelevantSelected,
      rationale: "The projection includes material the scenario declares irrelevant. The unnecessary exposure is observable, while any downstream model distraction remains empirical rather than proven here.",
    });
  }
  ...
  return {
    omission,
    inclusion,
    selection_pressure: omissionPressure && inclusionPressure
      ? "rebalance"
      : omissionPressure
        ? "expand"
        : inclusionPressure
