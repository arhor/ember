...
The cognition path commits provider completion and a payload-free expression occurrence before transient presentation, with `deliveryStatus=pending`. If the process dies after presentation may have happened but before the final `displayed` commit, recovery preserves `pending`.
