...
arguments from the shared AI execution seam. Issue
...
Application resolves capability bindings for this cognition
        |
        v
AiExecutionRequest + AiExecutionOptions(selected bindings)
        |
        v
...
selector. The adapter receives the already-bounded `AiExecutionRequest` plus only those
...
validation. The adapter immediately calls Ember's existing `validateAiExecutionResult`
...
```text
(AiExecutionRequest, AiExecutionOptions) -> AiExecutionResult
```
...
Capability bindings are deliberately not added to `AiExecutionRequest`. They cross the
...
AI SDK exposes considerably more operational data than belongs in a `AiExecutionResult`:
...
None of this evidence is copied into `AiExecutionResult` or canonical state. The only
...
`ProviderError` semantics. AI SDK classes do not escape through `AiExecutionResult`, the
...
`invalid_output` evidence, while the independent `validateAiExecutionResult` check remains
...
existing AI execution contract.
...
1. accept the already-selected `AiExecutionRequest` and per-call capability bindings
...
5. return an Ember `AiExecutionResult` rather than foreign SDK result/session types; and
6. pass `validateAiExecutionResult` before the result reaches canonical cognition state.
