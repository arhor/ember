...
10a now provides the Ember-owned `AiExecutor` contract in `src/ai/`, moves the shared AI
SDK cognition executor and Claude Code construction behind it, and leaves compatibility
aliases only for the Codex, Cursor, and process backends awaiting their parity slices.
...
| 6     | Cognition execution validates the prepared projection, then calls `AiExecutor(request, timeout/signal)`                     | Claude → shared SDK executor; transitional Codex/Cursor/process compatibility adapters currently bypass SDK. Only the already-built Ember projection and current input cross the seam             |
