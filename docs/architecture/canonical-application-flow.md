---
summary: "Issue #304 research proposal for one Ember application coordinator: production CLI/Telegram traces, explicit persistence and semantic ownership, AI SDK extraction, portable host boundaries, and the #303 migration sequence."
read_when:
...
flowchart TD
    ...
    A["app/application.ts: interact(event), lease + runtime episode"]
    ...
    R["app/application.ts: stop episode, release lease, return delivery ID"]
    O["app/delivery.ts: deliver(id), lease, reconcile, persist attempt"]
    T["surface transport callback: stdout or sendMessage"]
    E["app/delivery.ts: persist receipt/failure/uncertainty, release lease"]
    S --> A --> L --> C --> AI
    D --> C
    AI --> F --> X
    X --> F --> AI
...
2. Application acquires the injected writer lease, loads current state, and records an
   ...
   inference and post-turn work. Never hold it while waiting for the next CLI line or poll.
...
6. Stop the cognition runtime episode and release the lease in nested `finally` cleanup.
   Return references and diagnostics, never the whole canonical state.
7. The surface asks `app.deliver` to handle the returned ID. This separate use case acquires
   a lease, reloads/reconciles the intent, verifies its surface/destination binding, persists
   `started`, invokes the supplied transport function, and persists the observed outcome.
   Initially retain this lease across send, as today, so concurrent recovery cannot mistake
   a live attempt for an abandoned one. Releasing either long-held lease needs a separate
   currentness/attempt-ownership design and is not part of the extraction.
8. A delivery observation after episode stop updates delivery evidence without reopening
   the stopped runtime or extending its clean-stop timestamp. Telegram advances its polling
   offset only after its existing handled-outcome policy permits it. Pending deliveries are
   recovered through the same delivery use case, without re-running cognition or post-turn work.
...
| `providers/codex.ts`                                                                                                        | Prompt/result protocol, CLI flags, JSONL, thread/exit evidence             | `ai/providers/codex.ts` bounded model bridge + host process helper                                        | SPLIT       | `LanguageModelV3` bridge                              | #315b; delete direct ordinary invocation after parity                                               |
| `providers/cursor.ts`                                                                                                       | Prompt/JSON result, isolated CLI/session, exit evidence                    | `ai/providers/cursor.ts` bounded model bridge + host process helper                                       | SPLIT       | `LanguageModelV3` bridge                              | #315c; preserve unsupported-tool behavior                                                           |
| `providers/process.ts`                                                                                                      | Generic process protocol and provider label                                | Test/provider fixture adapter; reusable label helper at composition boundary                              | SPLIT       | SDK test model/bridge for ordinary fixtures           | #315, #318; retain explicit operator compatibility if still used, never default production fallback |
| `providers/evidence.ts`                                                                                                     | External-ID/evidence validation helpers                                    | `ai/providers/evidence.ts`                                                                                | MOVE        | —                                                     | #315; retain shared specialist consumers as appropriate                                             |
...
| `delegation/codex.ts`                                                                                                       | Specialist public integration entry                                        | Existing specialist boundary                                                                              | KEEP        | —                                                     | #317, #319                                                                                          |
| `delegation/codex-specialist.ts`                                                                                            | Envelope, child execution, records, reconciliation                         | Delegation semantics + external Codex execution/persistence collaborators                                 | SPLIT       | No ordinary tool-loop substitution                    | #317, #319; keep change limited to shared boundary dependencies                                     |
| `delegation/specialist-reintegration.ts`                                                                                    | Currentness/authority decisions and crash-consistent file evidence         | Delegation policy + app reintegration + persistence IO                                                    | SPLIT       | —                                                     | #317, #319; retain record correlation                                                               |
...
interface EmberApplication {
  interact(event: InteractionEvent, options?: { signal?: AbortSignal }): Promise<InteractionResult>;
...
or credentials. `pendingDeliveries` returns references scoped to the authenticated
binding; `deliver` revalidates the stored binding and current eligibility regardless of
what the caller supplies. Only an eligible durable attempt exposes retained text to
the transport callback. Unexpected callback throws become uncertain, as today.
...
Both run `interact` then `deliver` when a delivery exists, including replay results
whose delivery is pending. `deliver` performs the no-send checks itself. No surface
inspects ledger records to decide retry policy. An abandoned foreground process leaves
...
- Installed `@ai-sdk/provider/src/language-model/v3/` defines the model bridge used
  below: standardized prompt, `responseFormat`, tools, abort signal, `doGenerate`,
  `doStream`, and normalized results. These are adapter-local types.
...
implement thin `LanguageModelV3` bridges in #315b/#315c:
...
3. Normalize final JSON text, finish reason, usage when known, and warnings into the
   SDK model result. Unknown token usage stays unknown. Required `doStream` may emit a
   single final text chunk after the bounded process finishes until real incremental
   output is supported; this does not claim live token streaming.
...
If the bridges import `LanguageModelV3` directly from `@ai-sdk/provider`, add that
...
ai/providers -> host process helpers + external provider libraries
...
install Node/package/provider host bits
  ...
  -> CLI sends ordinary messages to app.interact, then app.deliver
...
| 7 — #312                    | Extract delivery use case and make interaction return committed delivery ID                                                                           | Intent before optional follow-up; attempt before send; confirmed/uncertain/missing-text recovery; delete output hooks/Writable from cognition |
...
| 21 — #321                   | Final deletion, documentation and zero-exception boundary audit                                                                                       | Update this proposal to implemented trace, source-layout and affected design/runbooks; no stale facades or second ordinary path               |
