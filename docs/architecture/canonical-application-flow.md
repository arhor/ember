...
memory/onboarding orchestration from #314 is also application-owned. Issue #315 slice
10a now provides the Ember-owned `AiExecutor` contract in `src/ai/` and moves the shared AI
SDK cognition executor and Claude Code construction behind it. Slice 10b routes ordinary
...
| 6     | Cognition execution validates the prepared projection, resolves application-selected capability bindings, then calls `AiExecutor(request, options)` | Claude/Codex/Cursor → shared SDK executor; only the generic process compatibility adapter bypasses SDK. The already-built Ember projection, current input, and selected bindings cross the seam   |
| 7     | SDK path: selected bindings → `tool` schemas → `createCapabilityExecutionFirewall` → integration                                                    | SDK loops; Ember authorizes, validates semantic arguments, fences repeated capability attempts, and classifies effects. Calendar action ledger persists consequential attempts                    |
...
| 12    | The application runs explicit onboarding-progress and conversation-memory follow-ups through `app/post-turn.ts`                                     | Follow-up failures remain observable diagnostics without changing completed cognition; memory generation records, deterministic adoption, provenance, and supersession semantics remain unchanged |
...
Default memory/progress helpers make additional typed SDK calls for backends with a
structured model bridge. They use their existing Ember-owned schemas and do not embed
control results in `ProviderResult.reply`. These are bounded control tasks, not
additional ordinary user interactions or independent agent identities. The generic
process compatibility backend uses its existing JSON-in-reply wrappers because its
external protocol represents only ordinary cognition results.
...
| `ai/cognition.ts`                                                                                                           | SDK invocation, tools, output, streaming, failure/evidence translation            | Implemented shared AI execution infrastructure                                                                                | MOVED       | `generateText`, `streamText`, `tool`, `Output.object` | #315a/#316 implemented                                                                |
| `ai/claude-code.ts`                                                                                                         | Subscription-backed model, isolated settings/workspace/auth                       | Implemented Claude model construction plus shared cognition executor                                                          | MOVED       | Existing `LanguageModel` provider                     | #315a implemented                                                                     |
| `providers/codex.ts`, `ai/codex.ts`                                                                                         | Codex CLI protocol plus bounded SDK model bridge                                  | Ordinary and typed-control SDK routes implemented; low-level protocol returns arbitrary schema output only to the bridge      | SPLIT       | `LanguageModelV4` bridge                              | #315b/#315d implemented                                                               |
| `providers/cursor.ts`, `ai/cursor.ts`                                                                                       | Cursor CLI protocol plus bounded SDK model bridge                                 | Ordinary and typed-control SDK routes implemented; isolated runtime remains tool-denied                                       | SPLIT       | `LanguageModelV4` bridge                              | #315c/#315d implemented                                                               |
...
| `memory/memory-proposal-generation.ts`, `ai/memory-proposals.ts`                                                            | Ember projection/generation/adoption semantics; SDK structured generator          | Implemented split between `memory/` semantics and `ai/` mechanics                                                             | SPLIT       | `generateText` + `Output.object`                      | #308, #314, #315d implemented                                                         |
| `memory/provider-memory-proposal-generator.ts`                                                                              | Production JSON-in-reply compatibility helper for the generic process backend     | Retain until the explicit process compatibility migration                                                                     | COLLAPSE    | —                                                     | #315d compatibility; remove in #318                                                   |
| `onboarding/progress-evaluator.ts`, `ai/onboarding-progress.ts`                                                             | Ember control contract and process compatibility; normalized SDK evaluator        | Implemented split between onboarding semantics and `ai/` mechanics                                                            | SPLIT       | `Output.object`                                       | #314/#315d implemented                                                                |
...
| `capabilities/execution.ts`                                                                                                 | Authority, argument, occurrence, bounded evidence firewall                        | Existing semantic capability boundary                                                                                         | KEEP        | `tool.execute` calls it                               | #316 implemented; remove no semantic guard                                            |
...
| `ai/opportunity.ts`                                                                                                         | Structured opportunity decision behind agency-owned contracts                     | Implemented AI infrastructure adapter                                                                                         | MOVE        | `generateText` + `Output.object`                      | #315d implemented                                                                     |
| `ai/codex-opportunity.ts`                                                                                                   | Codex composition adapter over shared typed opportunity evaluation                | Implemented AI infrastructure adapter                                                                                         | COLLAPSE    | `Output.object` through Codex bridge                  | #315d implemented                                                                     |
...
| --------------------------------------------------------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| Model call in `ai/cognition.ts`                                       | Shared SDK implementation                                  | `generateText`; application/runtime consume only the Ember-owned `AiExecutor` contract                                               |
...
| Memory structured generation in `ai/memory-proposals.ts`              | Implemented SDK boundary                                   | `generateText` + `Output.object`; proposal assessment and durable generation outcomes remain under `memory/`                         |
