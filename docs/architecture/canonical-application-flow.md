...
| 6     | Cognition execution validates the prepared projection, then calls `AiExecutor(request, timeout/signal)`                     | Claude/Codex/Cursor → shared SDK executor; only the generic process compatibility adapter bypasses SDK. The already-built Ember projection and current input cross the seam                       |
