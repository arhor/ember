...
| `memory/provider-memory-proposal-generator.ts`                                                                              | Legacy test-only JSON-in-reply compatibility helper                               | Production uses the typed generator in `memory-proposal-generation.ts`                                                        | COLLAPSE    | `Output.object`                                       | #315d implemented; remove helper with obsolete fixture cleanup                        |
| `onboarding/progress-evaluator.ts`                                                                                          | Ember control contract plus typed SDK evaluator; legacy test helper remains       | `core/onboarding-work.ts` contract + typed evaluator                                                                          | SPLIT       | `Output.object`                                       | #314/#315d implemented                                                                |
...
| `agency/codex-opportunity-evaluator.ts`                                                                                     | Codex composition adapter over shared typed opportunity evaluation                | Shared typed opportunity executor                                                                                             | COLLAPSE    | `Output.object` through Codex bridge                  | #315d implemented                                                                     |
