...
does the same for ordinary Cursor cognition; compatibility aliases remain only for the generic
process backend and transitional Codex/Cursor control calls.
...
| `providers/contract.ts`                                                                                                     | Transitional aliases for backends awaiting SDK bridges                     | Implemented Ember contract in `ai/contract.ts`; delete aliases after remaining backend slices                                 | SPLIT       | —                                                     | #315b/#315c                                                                           |
...
| `providers/codex.ts`, `ai/codex.ts`                                                                                         | Codex CLI protocol plus bounded SDK model bridge                           | Implemented ordinary SDK route; legacy wrapper retained only for typed-control migration                                      | SPLIT       | `LanguageModelV4` bridge                              | #315b implemented; delete control wrapper in #315d                                    |
| `providers/cursor.ts`, `ai/cursor.ts`                                                                                       | Cursor CLI protocol plus bounded SDK model bridge                          | Implemented ordinary SDK route; legacy wrapper retained only for typed-control/bootstrap migration                            | SPLIT       | `LanguageModelV4` bridge                              | #315c implemented; delete control wrapper in #315d                                    |
