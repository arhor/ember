...
Codex cognition through a bounded `LanguageModelV4` bridge beneath that executor; compatibility
aliases remain only for Cursor, the generic process backend, and transitional Codex control calls.
...
| 6     | Cognition execution validates the prepared projection, then calls `AiExecutor(request, timeout/signal)`                     | Claude/Codex → shared SDK executor; transitional Cursor/process compatibility adapters bypass SDK. Only the already-built Ember projection and current input cross the seam                       |
