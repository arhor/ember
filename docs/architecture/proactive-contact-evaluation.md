...
message string as proof of a good decision: every case reports the policy outcome,
basis, source evidence, delivery observation, and separate Ember/model assertions.
...
Live exit code `1` means an Ember-owned policy or persistence assertion failed;
exit code `2` means only the model observations missed their fixture expectations.
...
between decision and delivery, uncertain then confirmed delivery, repeated
opportunities, and contact after a relevant remembered-currentness observation.
