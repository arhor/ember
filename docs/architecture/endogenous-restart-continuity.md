...
- a superseded consequence remains historical and cannot reactivate the still-live
  commitment; and
...
- current and historical meanings preserve the commitment and consequence lifecycle;
...
uses the production Codex opportunity evaluator, whose provider invocation defaults
to an ephemeral thread and explicitly forbids prior threads and outside context:
