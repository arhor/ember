...
test("supersession should keep A historical and make B current when slot matches exactly", () => {
    // Given
    const { state, ids } = populatedState(),
        original = cloneState(findMeaning(state, ids.preference));
    // When
    const nextId = supersede(state, PRINCIPAL, ids.preference, "Prefer detailed architectural rationale"),
        next = findMeaning(state, nextId),
        changed = findMeaning(state, ids.preference);
    // Then
    assert.deepEqual(
        [
            changed.currentness,
...
test("optional detail should reject deletion label when fault reason claims deletion", async () => {
    ...
    const error = await captureError(() =>
        withholdDetail(state, PRINCIPAL, ids.detail, { reason: "privacy deletion" }),
    );
    // Then
...
test("recovery should report clean interval when prior runtime stopped explicitly", () => {
    ...
    const { state } = populatedState(),
        first = startRuntime(state, PRINCIPAL, SCOPE, { timestamp: "2026-08-29T10:00:00Z" }),
        stopped = stopRuntime(first.state, first.runtimeId, {
            reason: "explicit_cli_exit",
...
test("state validator should reject clean gap when restart clock precedes stop", async () => {
    // Given
    const { state } = populatedState(),
        first = startRuntime(state, PRINCIPAL, SCOPE, { timestamp: "2026-08-29T12:00:00Z" }),
        stopped = stopRuntime(first.state, first.runtimeId, {
            reason: "explicit_cli_exit",
    ...
    // When
    const error = await captureError(() =>
        startRuntime(stopped, PRINCIPAL, SCOPE, { timestamp: "2026-08-29T11:00:00Z" }),
    );
    // Then
...
test("ordinary projection should include current B and exclude historical A when scope matches", () => {
    // Given
    const { state, ids } = populatedState(),
        replacement = supersede(state, PRINCIPAL, ids.preference, "Prefer detailed architectural rationale"),
        started = startRuntime(state, PRINCIPAL, SCOPE, { timestamp: "2026-08-30T11:00:00Z" });
    // When
    ...
    // Then
    assert.deepEqual(
        [selected.includes(replacement), selected.includes(ids.preference), selected.includes(ids.fact)],
        [true, false, false],
    );
});
test("explain projection should label history provenance commitment and gap when IDs are requested", () => {
    ...
    const { state, ids } = populatedState(),
        replacement = supersede(state, PRINCIPAL, ids.preference, "Prefer detailed architectural rationale");
    withholdDetail(state, PRINCIPAL, ids.detail);
    const first = startRuntime(state, PRINCIPAL, SCOPE, { timestamp: "2026-08-29T10:00:00Z" }),
        stopped = stopRuntime(first.state, first.runtimeId, {
            reason: "explicit_cli_exit",
            timestamp: "2026-08-29T11:00:00Z",
        }),
        restarted = startRuntime(stopped, PRINCIPAL, SCOPE, { timestamp: "2026-08-30T11:00:00Z" });
    // When
    const projection = buildProjection(restarted.state, {
            principal: PRINCIPAL,
            scope: SCOPE,
            currentInput: "Explain",
            currentTime: "2026-08-30T11:00:01Z",
