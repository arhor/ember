...
test("context harm rubric should distinguish omission, distraction, stale context, privacy boundaries, and truthful degradation", () => {
    // Given / When
    const omission = evaluateContextHarm(context({
        relevant: ["needed"],
        relevantNotSelected: ["needed"],
    }));
    const forbidden = evaluateContextHarm(context({
        forbidden: ["private"],
        forbiddenSelected: ["private"],
    }));
    const stale = evaluateContextHarm(context({
        superseded: ["old"],
        supersededSelected: ["old"],
    }));
    const historical = evaluateContextHarm(context({
        relevant: ["old"],
        superseded: ["old"],
        supersededSelected: ["old"],
    }));
    const degraded = evaluateContextHarm(context({
        relevant: ["episode"],
        unavailable: ["episode"],
        unavailableSelected: ["episode"],
        unavailableWithGap: ["episode"],
    }));
    const distraction = evaluateContextHarm(context({
        irrelevant: ["noise"],
        irrelevantSelected: ["noise"],
    }));
    const rebalance = evaluateContextHarm(context({
        relevant: ["needed"],
        irrelevant: ["noise"],
        relevantNotSelected: ["needed"],
        irrelevantSelected: ["noise"],
...
test("representative longitudinal corpus should show inclusion pressure without observed relevant omission", async () => {
    ...
    for (const report of reports.values()) {
        for (const episode of report.episodes) {
            ...
            assert.equal(harm.inclusion.findings.some(item => item.kind === "forbidden_inclusion"), false);
        }
    }
    ...
    assert.equal(memoryHarm.inclusion.findings.some(item => item.kind === "purposeful_historical_inclusion"), true);
    assert.equal(memoryHarm.inclusion.findings.some(item => item.kind === "truthful_degraded_inclusion"), true);
    assert.equal(memoryHarm.inclusion.findings.find(item => item.kind === "irrelevant_inclusion")?.meanings.length, 64);
