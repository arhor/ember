...
test("parses documented frontmatter subset and preserves hint order", () => {
  const { metadata, body } = parseFrontmatter(
    docText({ readWhen: ["First task", "Second task"], body: "# Body\n" }),
  );
...
test("validates supersession target and cycle", () => {
  withRepo((root) => {
    writeDoc(
      root,
      "docs/old.md",
      docText({ status: "superseded", supersededBy: "docs/new.md", role: "design" }),
    );
    ...
    writeDoc(
      root,
      "docs/new.md",
      docText({ status: "superseded", supersededBy: "docs/old.md", role: "design" }),
    );
...
test("filters default, deep, and all without semantic matching", () => {
  ...
  assert.deepEqual(selectDocuments(documents).map((doc) => doc.path), ["docs/a.md", "docs/b.md"]);
  assert.deepEqual(selectDocuments(documents, { deep: true }).map((doc) => doc.path), [
    "docs/a.md",
    "docs/b.md",
    "docs/c.md",
    "docs/d.md",
...
test("explicit exclusions are applied by exact repository path", () => {
  withRepo((root) => {
    ...
    assert.deepEqual(validation.errors, []);
    assert.deepEqual(loaded.documents.map((doc) => doc.path), ["docs/include.md"]);
  });
});
...
test("missing metadata cannot silently disappear from corpus", () => {
  withRepo((root) => {
    ...
    const validation = validateCorpus(loaded.documents, loaded.errors);
    assert.deepEqual(loaded.documents.map((doc) => doc.path), ["docs/good.md"]);
