...
const deepPaths = new Set(selectDocuments(loaded.documents, { deep: true }).map((document) => document.path));
const allPaths = new Set(selectDocuments(loaded.documents, { allDocuments: true }).map((document) => document.path));
...
test("representative roles and supersession match governance", () => {
    ...
    assert.equal(byPath.get("docs/architecture/decisions/0001-continuity-belongs-to-ember.md").role, "decision");
    ...
    assert.equal(byPath.get("docs/research/source-material/memory-and-remembering-deep-research.md").role, "source");
