...
test("state store should round trip complete document when commit succeeds", async () => {
    ...
    // When
    const committed = await store.commit(0, cloneState(state));
...
test("state store should reject stale candidate when revision changed", async () => {
    ...
    // When
    const error = await captureError(() => store.commit(0, cloneState(state)));
    await store.releaseWriteLease(lease);
    // Then
...
test("state store should report uncertain durability when directory sync fails after replace", async () => {
    ...
    // When
    const error = await captureError(() => store.commit(0, cloneState(state)));
    const exposed = await store.load();
    await store.releaseWriteLease(lease);
    // Then
...
test("write lease should refuse second writer when live PID owns lock", async () => {
    ...
    const directory = await tempDir(),
        path = join(directory, "ember.json"),
        first = new StateStore(path),
        second = new StateStore(path);
    const lease = await first.acquireWriteLease();
    // When
    const error = await captureError(() => second.acquireWriteLease());
...
test("write lease should fail closed without probing when lock PID is unsafe", async () => {
    ...
    const store = new StateStore(path, { hostname: "h", kill: (...args) => calls.push(args) });
    // When
...
test("write lease should classify metadata malformed when acquired timestamp is calendar-invalid", async () => {
    ...
    await writeFile(
        ...
        JSON.stringify({
            ...
            pid: 123,
            hostname: "h",
            acquired_at: "2026-02-31T10:00:00Z",
        }),
    );
    const store = new StateStore(path, { hostname: "h" });
    // When
    const status = await store.lockStatus();
    // Then
...
test("write lease should preserve lock when owner token changes before release", async () => {
    ...
    await writeFile(`${path}.lock`, JSON.stringify({ ...lease.metadata, owner_token: "different" }));
    // When
...
test("stale lock quarantine should rename artifact when token and quiescence are confirmed", async () => {
    ...
    const directory = await tempDir(),
        ...
        metadata = {
            lock_version: 1,
            owner_token: "expected",
            pid: 123,
            hostname: "h",
            acquired_at: "2026-08-29T10:00:00Z",
        };
    await writeFile(`${path}.lock`, JSON.stringify(metadata));
    const store = new StateStore(path, {
        hostname: "h",
        kill: () => {
            const e = new Error();
...
test("stale lock quarantine should refuse mutation when quiescence is not confirmed", async () => {
    ...
    const error = await captureError(() =>
        store.quarantineStaleLock({ ownerToken: "expected", confirmQuiescent: false }),
    );
    // Then
    assert.ok(error instanceof ConcurrentWriter);
});
test("stale lock quarantine should preserve availability failure when PID is reused", async () => {
    ...
    const store = new StateStore(path, { hostname: "h", kill: () => {} });
    // When
    const error = await captureError(() =>
        store.quarantineStaleLock({ ownerToken: "expected", confirmQuiescent: true }),
    );
    // Then
    assert.equal(error.diagnosis.status, "live");
});
