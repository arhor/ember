...
test("longitudinal currentness pressure should preserve corrections as history and keep unresolved reports distinct", async () => {
    ...
    assert.equal(conflictMeanings.every(item => item.currentness === "current"), true);
    assert.equal(conflictMeanings.every(item => item.supersedes === null && item.superseded_by === null), true);
    assert.notEqual(conflictMeanings[0]!.source_evidence[0]!.evidence_id, conflictMeanings[1]!.source_evidence[0]!.evidence_id);
    assert.equal(conflictMeanings.every(item => item.source_evidence[0]!.source_actor === "user:user-1"), true);
    ...
    assert.equal(history.projection.meanings.some(item => item.content.includes("CURRENTNESS_FACT_OLD")), true);
    assert.equal(history.projection.meanings.some(item => item.content.includes("CURRENTNESS_FACT_NEW")), true);
});
