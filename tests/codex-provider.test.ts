...
describe("Codex provider", () => {
    ...
    test("Codex AI SDK bridge should honor the call timeout when model configuration is longer", async () => {
        // Given
        const { request } = requestFixture();
        const fixture = childDouble({ closeOnKill: true });
        const executor = createAiSdkCognitionExecutor(
...
describe("executeCognition", () => {
    test("should retain external thread ID only as operational episode evidence when Codex completes", async () => {
        ...
        // When
        const result = await executeCognition(createFileBackedRepositoriesForState(store), loaded, {
            runtimeId,
            principal: PRINCIPAL,
            scope: SCOPE,
            text: "hello",
