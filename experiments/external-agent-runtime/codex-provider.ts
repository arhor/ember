...
export async function invokeCodexExec(request: ProviderRequest, options: CodexOptions): Promise<ProviderResult> {
    ...
    if (terminal.code !== 0)
        throw new Error(
            `Codex exited with ${terminal.signal ? `signal ${terminal.signal}` : `status ${terminal.code}`}${diagnostic || stdoutText.trim() ? `: ${diagnostic || stdoutText.trim()}` : ""}`,
        );
...
async function readRequest() {
    ...
    if (
        ...
        typeof request.cognition_id !== "string" ||
        !isRecord(request.projection) ||
    ...
    Promise.resolve()
        .then(async () => {
            ...
            if (options.captureRequest)
                await writeFile(options.captureRequest, `${JSON.stringify(request, null, 2)}\n`, {
                    encoding: "utf8",
                    mode: 0o600,
                });
