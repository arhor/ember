...
const relationshipId = rememberRelationship(initial, PRINCIPAL, `relationship:${PRINCIPAL}`, SCOPE, "Synthetic issue-44 collaborator fixture");
const factId = rememberFact(initial, PRINCIPAL, `user:${PRINCIPAL}`, "fixture-server", SCOPE, "The synthetic fixture server uses EmberBoard 42 hardware");
rememberPreference(initial, PRINCIPAL, `user:${PRINCIPAL}`, "unrelated-preference", "project:unrelated", EXCLUDED_MARKER);
