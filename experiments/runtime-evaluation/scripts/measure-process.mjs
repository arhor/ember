...
async function treeRss(rootPid) {
  ...
  let entries;
  try { entries = await readdir("/proc", { withFileTypes: true }); } catch { return await rss(rootPid); }
  ...
  while (changed) {
    changed = false;
    for (const [pid, ppid] of processes) if (wanted.has(ppid) && !wanted.has(pid)) { wanted.add(pid); changed = true; }
  }
  let total = 0;
...
function summarize(values) {
  ...
  return { median: round(median), min: round(sorted[0]), max: round(sorted.at(-1)), samples: values.map(round) };
}
function round(value) { return Math.round(value * 100) / 100; }

function parseArgs(args) {
  const options = { label: "measurement", repeat: 5, warmup: 1, cwd: process.cwd(), stdinText: null, stdinDelayMs: 0, command: [] };
