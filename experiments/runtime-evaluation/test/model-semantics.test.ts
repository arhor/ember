...
import { describeMeaning, evidenceId, fixtureState, meaningId } from "../src/model.ts";
...
test("ordinary and explain projections stay purpose and scope bounded", () => {
    ...
    const explained = buildProjection(updated, "explain", activeInput, [meaningId("meaning-preference-a")]);
    assert.ok(explained.selection.meaning_ids.includes(meaningId("meaning-preference-a")));
    assert.ok(explained.selection.meaning_ids.includes(meaningId("meaning-preference-a-successor")));
