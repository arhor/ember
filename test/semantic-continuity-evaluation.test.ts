...
const semanticAuditProvider: HarnessProvider = async invocation => {
  ...
  const currentPreference = projection.meanings.find(item => item.kind === "preference" && item.currentness === "current");
  const historicalPreference = projection.meanings.find(item => item.kind === "preference" && item.currentness === "superseded");
...
test("issue 56 supersession scenario should preserve currentness, commitment, gap, and epistemic restraint", async () => {
  ...
  assert.equal(explained.projection.meanings.some(item => item.kind === "preference" && item.currentness === "current" && item.content === "Prefer detailed semantic reports"), true);
  assert.equal(explained.projection.meanings.some(item => item.kind === "preference" && item.currentness === "superseded" && item.content === "Prefer terse semantic reports"), true);
  assert.equal(explained.projection.meanings.some(item => item.kind === "commitment" && item.prospective_lifecycle === "live" && item.applicability === "last_known_live_needs_currentness_check"), true);
  assert.equal(explained.projection.gaps.some(item => item.gap_kind === "unavailable_detail"), true);
  assert.equal(JSON.stringify(explained.projection).includes("HIDDEN_NICKNAME_56"), false);

  const ordinary = report.episodes[2];
  assert.equal(ordinary.projection.meanings.some(item => item.content === "Prefer terse semantic reports"), false);
  assert.equal(ordinary.projection.meanings.some(item => item.content === "Prefer detailed semantic reports"), true);
  assert.equal(ordinary.projection.meanings.some(item => item.kind === "commitment" && item.prospective_lifecycle === "live"), true);
});
...
test("issue 56 provenance scenario should keep user testimony distinct from Ember-owned commitment across restart", async () => {
  ...
  for (const episode of report.episodes) {
    ...
    assert.deepEqual(commitment.source_evidence.map(item => item.source_role), ["ember_adoption", "user_command"]);
    assert.equal(commitment.owner, "ember");
  }

  assert.equal(report.episodes[1].projection.meanings.some(item => item.kind === "commitment" && item.applicability === "last_known_live_needs_currentness_check"), true);
  assert.equal(report.episodes.every(episode => episode.model_observations.every(item => item.passed)), true);
});
