...
test("process-restart harness should preserve Ember continuity while Codex threads stay fresh", async () => {
  ...
  assert.equal(report.ember_assertions.find(item => item.assertion === "complete Ember process restarted")?.passed, true);
  assert.equal(report.ember_assertions.find(item => item.assertion === "lineage preserved across process restart")?.passed, true);
  assert.equal(report.ember_assertions.find(item => item.assertion === "restart recovery records no supported cognition during downtime")?.passed, true);
  assert.equal(report.ember_assertions.find(item => item.assertion === "fresh provider thread changed across process restart")?.passed, true);
...
test("process-restart harness should reject accidental Codex thread reuse across processes", async () => {
  ...
  assert.equal(report.ember_assertions.find(item => item.assertion === "fresh provider thread changed across process restart")?.passed, false);
