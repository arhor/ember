...
test("external runtime probe should report only direct-child termination when cancellation is requested", async () => {
  ...
  // Then
  assert.deepEqual([result.termination_reason, result.direct_child_exit_observed, result.exit_signal], ["explicit_cancel", true, "SIGTERM"]);
});
...
test("external runtime probe should distinguish timeout when deadline triggers termination", async () => {
  ...
  // When
  const result = await runProbe({ command: process.execPath, arguments_: ["-e", script], cwd: process.cwd(), timeoutMs: 20 });
...
test("external runtime probe should report unconfirmed when direct child termination is never observed", async () => {
  ...
  // When
  const result = await runProbe({ command: "fixture", arguments_: [], cwd: process.cwd(), timeoutMs: 100, cancelAfterMs: 5, terminationGraceMs: 5, finalTerminationMs: 20, spawnImpl: () => child });

  // Then
  assert.deepEqual([result.termination_reason, result.direct_child_exit_observed, result.exit_code, result.exit_signal], ["explicit_cancel", false, null, null]);
});
...
test("Codex experiment adapter should return validated provider result when bounded Ember request is supplied", async () => {
  ...
  const projection = buildProjection(started.state, { principal: PRINCIPAL, scope: SCOPE, currentInput: "Which server?", currentTime: "2026-08-31T10:00:01Z", runtimeId: started.runtimeId });
  const request = { contract_version: 1 as const, cognition_id: "cognition-adapter-test" as const, projection, input: { text: "Which server?" } };
  ...
  // When
  const result = await invokeCodexExec(request, { command: process.execPath, argumentPrefix: ["-e", fakeCodex], cwd: await tempDir() });

  // Then
  assert.deepEqual([result, buildCodexPrompt(request).includes(JSON.stringify(request))], [{ contract_version: 1, reply: "bounded answer", used_meaning_ids: [used] }, true]);
});
