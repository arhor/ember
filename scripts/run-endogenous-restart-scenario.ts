...
function parseArguments(args: string[]) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (name === "--scenario" && value) { scenarioPath = isAbsolute(value) ? value : resolve(value); index += 1; }
    else if (name === "--scenario-id" && value) { scenarioId = value; index += 1; }
    else if (name === "--codex-command" && value) { codexCommand = value; index += 1; }
    else if (name === "--timeout-seconds" && value && Number(value) > 0) { timeoutSeconds = Number(value); index += 1; }
    else if (name === "--report" && value) { report = isAbsolute(value) ? value : resolve(value); index += 1; }
    else throw new Error("usage: run-endogenous-restart-scenario.ts [--scenario PATH] [--scenario-id ID] [--codex-command PATH] [--timeout-seconds N] [--report NEW_PATH]");
  }
