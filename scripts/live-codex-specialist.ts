...
if (process.env.EMBER_RUN_LIVE_SPECIALIST !== "1") {
  ...
  const spec = createSpecialistEpisode({
    ...
    runtime_policy: { command: "codex", argument_prefix: [], sandbox: "workspace-write", network: "no_additional_grant", configuration: "isolated", environment: "allowlisted_runtime_auth", timeout_seconds: 180, stdout_limit_bytes: 1024 * 1024, session_mode: "ephemeral" },
  ...
  try {
    ...
    assert.deepEqual([record.runtime_state, record.report_state, record.report?.objective_disposition], ["exited", "reported_success", "completed"], `Codex did not return a successful report: ${JSON.stringify(record.observations)}`);
    const expected = [...baseline, ["greeting.txt", "file", Buffer.from("hello from bounded Codex specialist\n").toString("base64")]].sort((left, right) => left[0]! < right[0]! ? -1 : left[0]! > right[0]! ? 1 : 0);
    assert.deepEqual(await workspaceSnapshot(workspace), expected, "controlled workspace contained a collateral or unexpected change");
