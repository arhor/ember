...
export function validateCorpus(documents, errors = []) {
  ...
  for (const document of documents) {
    ...
    if (target.discoveryStatus === "historical") {
      validationErrors.push(
        `${document.path}: superseded_by target '${targetPath}' cannot be historical`,
      );
    }
  }
  ...
  for (const [hint, owners] of [...hintOwners.entries()].sort(([a], [b]) => compareStrings(a, b))) {
    if (owners.length > 1) {
      warnings.push(
        `duplicate read_when hint across documents: '${hint}' (${owners.sort().join(", ")})`,
      );
    }
  }
...
export function selectDocuments(documents, { deep = false, allDocuments = false } = {}) {
  ...
  return ordered.filter(
    (document) => document.discoveryStatus === "current" && allowedRoles.has(document.role),
  );
}
