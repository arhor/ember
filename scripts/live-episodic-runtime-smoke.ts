...
async function main() {
  ...
  try {
    ...
    assert.equal(wakeTerminal.kind, "completed", `wake worker did not complete cleanly: ${wakeTerminal.detail ?? wakeTerminal.kind}`);
    assert.equal(wakeTerminal.evaluator_failure, null, `wake evaluator failed: ${wakeTerminal.evaluator_failure}`);
    assert.ok(["cognition", "defer", "no_cognition"].includes(String(wakeTerminal.decision)), "wake did not persist a valid decision");
    assert.ok(await records.wakeObservation(wakeId, "dispatching"), "wake never crossed the durable dispatching boundary");
    ...
    const spec = createSpecialistEpisode({
      ...
      context_projection: [{
        content: "This is an isolated temporary workspace used only for Ember episodic runtime live validation.",
    ...
    assert.ok(await records.specialistObservation(specialistId, "worker_started"), "specialist worker never established worker_started");
    assert.ok(await records.specialistObservation(specialistId, "worker_completed"), "specialist worker never established worker_completed");
...
async function waitForWake(records: EpisodicRecordStore, wakeId: string, timeoutMs: number): Promise<RuntimeObservation> {
